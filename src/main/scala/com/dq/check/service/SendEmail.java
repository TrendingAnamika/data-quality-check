...
public class SendEmail
{
    ...
    public void sendReport(Map<String, String> mail_config){
        ...
        String[] to = mail_config.get("dq_results_sent_to").split(",");
        System.out.println("Inside email routine: Send To: "+mail_config.get("dq_results_sent_to"));
        ...
        try{
            ...
            switch(mail_config.get("exec_mode")) {
                case "dq":
                    System.out.println("---Inside email routine and the execution mode is: dq");
                    String fileName = mail_config.get("hdfs_path_for_dq_result");
                    //if (fileName.contains("csv")) {
                    System.out.println("hdfs dfs -get " + fileName + "part*.csv");
                    processBuilder.command("bash", "-c", "hdfs dfs -get ".concat(fileName.concat("part*.csv")));
                    //}
                    /*else { //To handle Excel attachments as they get saved as .xls without part files
                        System.out.println("hdfs dfs -get " + fileName);
                        processBuilder.command("bash", "-c", "hdfs dfs -get ".concat(fileName));
                    }*/
                    ...
                    if (exitVal != 0) {
                        ...
                        System.out.println("Failed to download the DQ report file from hdfs");
                        BufferedReader stdError = new BufferedReader(new InputStreamReader(process.getErrorStream()));
                        String errMsg = null;
                        while ((errMsg = stdError.readLine()) != null) {
                            System.out.println("DQ file download failed - Error Message: " + errMsg);
                            System.exit(1);
                        }
                    }

                    //---- Handle DQ attachments ----
                    MimeBodyPart messageBodyPart2 = new MimeBodyPart();
                    //if (fileName.contains("csv")) { //rename the part files into provided csv
                    processBuilder.command("bash", "-c", "mv part*.csv ".concat(mail_config.get("dataset_name").concat(".csv")));
                    //}
                    Process process_fl_rnm = processBuilder.start();

                    int exitVal_fl_rnm = process_fl_rnm.waitFor();
                    if (exitVal_fl_rnm == 0) {
                        String filename = mail_config.get("dataset_name").concat(".csv");
                        ...
                        System.out.println("------ DQ mail sent -----");
                        ...
                        processBuilder.command("bash", "-c", "rm ".concat(mail_config.get("dataset_name").concat(".csv")));
                        Process remove_process = processBuilder.start();
                        int remove_exitVal = remove_process.waitFor();
                        if (remove_exitVal != 0) {
                            System.out.println("WARN: Could not remove the file downloaded from hdfs: " + mail_config.get("dataset_name").concat(".csv"));
                        }
                        else
                        {
                            System.out.println("INFO: Removed the following mail attachment file(s) downloaded from hdfs: " + mail_config.get("dataset_name").concat(".csv"));
                        }

                    } else {
                        System.out.println("Failed to rename the DQ report downloaded from hdfs");
                        BufferedReader stdError = new BufferedReader(new InputStreamReader(process_fl_rnm.getErrorStream()));
                        String errMsg = null;
                        while ((errMsg = stdError.readLine()) != null) {
                            System.out.println("DQ file rename Error Message: " + errMsg);
                        }
                    }
                    break;
            ...
            e.printStackTrace();
            mailRoutineError("Failure in sendReport email Routine.\n\nPlease check spark yarn log for further details\n\nspark application id can be fetched from Autosys log",  "Failure in sendReport email Routine: Unable to send mail",  mail_config.get("dq_error_sent_to"));
        }
    ...
    public void errorMail(Map<String, String> mail_config, String err_msg){
        ...
        //Get the session object
        Properties properties = System.getProperties();
        String[] to = mail_config.get("dq_error_sent_to").split(",");
        properties.setProperty("mail.smtp.host", host);
        Session session = Session.getDefaultInstance(properties);
        ...
        try {
            ...
            */
            ...
            mailRoutineError("Failure in errorMail Routine" +
                    ".\n\nPlease check spark yarn log for further details\n\nspark application id can be fetched from Autosys log",  "Failure in errorMail Routine: Unable to send mail",  mail_config.get("dq_error_sent_to"));
        }
    }
